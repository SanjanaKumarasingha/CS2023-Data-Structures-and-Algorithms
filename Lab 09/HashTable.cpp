...
struct HashTable{
    ...
    int hashfunc(string user_name){
        ...
        int hash = 0;
	    // add your code below
        // sum of the ASCII values of each character
        for (int i = 0; i < user_name.length(); i++) {
			sum = sum + int(user_name[i]);
		}

        hash = sum % MAX_LENGTH; // hash function
    ...
    void insert(string user_name,string user_password){
        ...
        if (empty) {
			password[hash] = user_password; // assigns the value of user_password to an element in the password array using hash as the index (key).
			cout<<"User added\n";
		}
        else {
    ...
    void hash_lookup(string user_name){
        ...
        hash = hashfunc(user_name);
        empty = is_slot_empty(hash);
	    // if hash (key) index value is not empty print the password
        if (!empty)
            cout<<"Password of "<< user_name <<" is : "<< password[hash] <<"\n";
        // if hash (key) index value is empty
        else
